...
public class IolistServiceImplV1 implements IolistService {
	...
	public void printList(List<IolistDto> iolists) {
		...
		System.out.println("SEQ\t거래일자\t거래시각\t고객ID\t고객명\t" + "상품코드\t상품명\t판매단가\t수량\t판매합계");
		System.out.println(Line.sLine(100));
	...
	protected Map<String, String> getDateTime() {
		...
		/*
		 * Java 의 Map<?,?> class 를 사용하여 임시 Dto 객체 만들기
		 * Map class 이용한 객체는 key, value 가 쌍으로 이루어진 데이터 객체
		 */
		// String type 의 key, 
		// String type 의 value 를 갖는 임시 Dto 객체
	...
	public void input() {
		while (true) {
			...
			String message = String.format("조회한 고객정보 %s, 고객이름 %s",
			...
			while(true) {
				...
				try {
					...
					HelpMessage.ERROR(
							String.format("수량을 정수로 입력하세요 (%s)", strQuan));
					continue;
				}
				break;
			}
		...
		while(true) {
			...
			try {
				...
				if(result.equals(strDate)) {
					...
					HelpMessage.ERROR(
							String.format("날짜가 유효범위를 벗어났습니다", strDate)
							);
				}
