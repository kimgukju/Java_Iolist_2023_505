...
public class BuyerServiceImplV2 extends BuyerServiceImplV1{
	...
	public BuyerDto findByBuId() {
		while(true) {
			...
			if(buyerDto == null) {
				HelpMessage.ERROR(
					String.format("입력한 ID 의 고객정보가 없습니다(%s)", strBuId));
				continue;
			}
			return buyerDto;
		}
	...
	protected String inputBuId() {
		while(true) {
			...
			if(strBuId.isBlank()) {
				HelpMessage.CONFIRM("고객ID 가 없습니다", "고객ID 를 새로 생성할까요?(Y:생성)");
			...
			try {
				...
				HelpMessage.ERROR(String.format("고객ID 는 정수로만 입력해야합니다(%s)", strBuId));
				continue;
			}
		}
	}
	...
	public void insert() {
		...
		String okMessage = String.format("고객ID : %s\n"
									...
									+"전화번호 : %s\n"
									+"주소 : %s\n",
									buyerDto.buId, buyerDto.buName, buyerDto.buTel, buyerDto.buAddr);
